...
cols = st.columns(4)
cols[0].image((img_sm*255).astype("uint8"), caption="Entrada RGB", use_container_width=True)
cols[1].image((gt_sm*255).astype("uint8"), caption="Máscara Real", use_container_width=True)
cols[2].image((pred_sm*255).astype("uint8"), caption="Predicción", use_container_width=True, clamp=True)
cols[3].image(((pred_sm>thr)*255).astype("uint8"), caption=f"Umbral {thr}", use_container_width=True)
